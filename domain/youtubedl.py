...
class Youtubedl:
    ...
    def __get_available_formats(self, youtube_url):
        try:
            command = [self.name] + self.video_format_options + [youtube_url]
    ...
    def get_url(self, cookie, youtube_url):
        try:
            p = subprocess.Popen([self.name] + self.filesystem_options + self.verbosity_options + [youtube_url],
